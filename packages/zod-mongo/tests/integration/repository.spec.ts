...
describe('repository integration', () => {
  ...
  describe('session() — non-transactional smoke', () => {
    ...
    it('insert via session() round-trips data readable via session-less repo', async () => {
      // Arrange
      const sessionDb = client.db('test-session-insert');
      const repo = createRepository(SessionCollection, sessionDb);
    ...
    it('find via session() returns same data as session-less find', async () => {
      // Arrange
      const sessionDb = client.db('test-session-find');
      const repo = createRepository(SessionCollection, sessionDb);
...
describe('session() — withTransaction (replica set)', () => {
  // ponytail: separate container without directConnection so the driver negotiates RS membership
  // and accepts withTransaction(). The main suites use directConnection: true which bypasses RS
  // negotiation and causes "Transaction numbers are only allowed on a replica set member" errors.
  let container: StartedMongoDBContainer;
  let client: MongoClient;
  ...
  beforeAll(async () => {
    container = await new MongoDBContainer('mongo:7')
      // ponytail: advertise as localhost so the driver can resolve the RS member host
      // from outside Docker. Without this, MongoDB announces its container hostname
      // (e.g. "a1b2c3d4e5f6") and the driver fails with ENOTFOUND on RS negotiation.
      .withCommand([
        'mongod',
        '--replSet',
        'rs0',
